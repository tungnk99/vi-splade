...
def train_splade_model():
    ...
    splade_model = SpladeModel(
        ...
        architecture="inference-free",  # Best for retrieval
        pooling_strategy="max",
        eval=False  # Training mode
    )
    print(f"   ✅ Model: {splade_model.model_name_or_path}")
    print(f"   ✅ Architecture: {splade_model.architecture}")
...
def quick_training_demo():
    ...
    splade_model = SpladeModel(
        ...
        architecture="splade",  # Simpler architecture
        pooling_strategy="max",
        eval=False
    )
...
splade_model = SpladeModel(
    model_name_or_path="vinai/phobert-base-v2",
    architecture="inference-free",
    eval=False
)

# 2. Create trainer
trainer = SpladeTrainer(model=splade_model)

# 3. Train with auto-resume
model = trainer.train(
...
model = trainer.resume_training(
    corpus_path="corpus.csv",
    test_path="test.csv",
    checkpoint_path="models/output/checkpoint-500"
)

# 5. Checkpoint management
checkpoints = trainer.list_checkpoints()
latest = trainer.find_latest_checkpoint()
trainer.clean_old_checkpoints(keep_last_n=2)
    """)
